...
log1.logSpeed = random.randint(1,5)
log2.logSpeed = random.randint(1,5) * -1
log3.logSpeed = random.randint(1,5)
log4.logSpeed = random.randint(1,5) * -1
...
def movelogs():
    ...
    for l in logList:
        ...
        if l.logRect.x >= 1740:
            l.logRect.x = 1590
            l.logSpeed = random.randint(5, 10)
...
def game_loop():
    ...
    while not gameOver:
        for e in event.get():
            ...
            if e.type == QUIT:
                ...
                if e.key == K_LEFT and frog.x + px > -10:
                    frog.x -= 60
                if e.key == K_RIGHT and frog.x + frog.w + px < width - 20:
                    frog. x += 60
                if e.key == K_DOWN and frog.y + py > 0:
                    frog.y += 65
                if e.key == K_UP and frog.y + frog.h + py < height - 147:
                    frog.y -= 65
                if e.key == K_p:
                    pause = True
        ...
        for l in logList:
            ...
            if onlog == 1 and frog.x > width or frog.x < 0:
                lives = lives - 1
                frog.x = x
                frog.y = y
        ...
        #drowning
            if onlog == 0 and count >= 7:
                px = 0
                count = 0

            if frog.y < 355 and onlog == 0 and count >= 6:
                lives-= 1
                frog.x = x
                frog.y = y
                count = 0
